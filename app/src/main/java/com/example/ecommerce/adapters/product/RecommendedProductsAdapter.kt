...
class RecommendedProductsAdapter(
    val context: Context,
    val flashSaleList: ArrayList<ProductDetailResponse.Recommended>,
    val currency: String
//    val bannerUrls: ArrayList<String>,
//    var delegate: ServicesFragment.IAdapter
) : RecyclerView.Adapter<RecommendedProductsAdapter.TrendingServicesVH>() {
...
//        return 5
    ...
    override fun onBindViewHolder(holder: TrendingServicesVH, position: Int) {
        ...
        val nowPrice = "$currency${response.price}"
        holder.binding.tvProductPrice.text = nowPrice

        // Strike through off price

        if (!response.price.equals(response.originalPrice)) {
            val originalPrice = "$currency${response.price}"
